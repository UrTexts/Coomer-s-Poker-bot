...
async def on_message(message):
    ...
    # If the message is from a DM (direct message)
    if message.guild is None:
        # Handle DM-specific commands (for example, commands in DM are treated differently)
        pass  # Add any specific DM logic here if needed

    # If the message is from a text channel in a server
    if isinstance(message.channel, discord.TextChannel):
        ...
        if command[0] == '!':
            ...
            # Special handling for the !deal command, to tell players their hands
            if command == '!deal' and messages[0] == 'The hands have been dealt!':
                await game.tell_hands(client)

            # Send the messages back to the channel
            await message.channel.send('\n'.join(messages))
            
# make sure yo put your token in here other wise it wot wont work.       
